...
return new(P||(P=Promise))(function(resolve,reject){function fulfilled(value){try{step(generator.next(value));}
catch(e){reject(e);}}
...
has(cacheName){return this.original.has(`${this.cacheNamePrefix}:${cacheName}`);}
...
match(request,options){return this.original.match(request,options);}
...
normalizeUrl(url){const parsed=this.parseUrl(url,this.scopeUrl);return(parsed.origin===this.origin?parsed.path:url);}
parseUrl(url,relativeTo){const parsed=!relativeTo?new URL(url):new URL(url,relativeTo);return{origin:parsed.origin,path:parsed.pathname,search:parsed.search};}
...
class CacheDatabase{constructor(adapter){this.adapter=adapter;this.cacheNamePrefix='db';this.tables=new Map();}
...
'delete'(key){return this.cache.delete(this.request(key),this.cacheQueryOptions);}
...
var UpdateCacheStatus=(function(UpdateCacheStatus){UpdateCacheStatus[UpdateCacheStatus["NOT_CACHED"]=0]="NOT_CACHED";UpdateCacheStatus[UpdateCacheStatus["CACHED_BUT_UNUSED"]=1]="CACHED_BUT_UNUSED";UpdateCacheStatus[UpdateCacheStatus["CACHED"]=2]="CACHED";return UpdateCacheStatus;})({});class SwCriticalError extends Error{constructor(){super(...arguments);this.isCritical=true;}}
function errorToString(error){if(error instanceof Error){return `${error.message}\n${error.stack}`;}
else{return `${error}`;}}
class SwUnrecoverableStateError extends SwCriticalError{constructor(){super(...arguments);this.isUnrecoverableState=true;}}
...
function _sha1(words32,len){const w=[];let[a,b,c,d,e]=[0x67452301,0xefcdab89,0x98badcfe,0x10325476,0xc3d2e1f0];words32[len>>5]|=0x80<<(24-len%32);words32[((len+64>>9)<<4)+15]=len;for(let i=0;i<words32.length;i+=16){const[h0,h1,h2,h3,h4]=[a,b,c,d,e];for(let j=0;j<80;j++){if(j<16){w[j]=words32[i+j];}
else{w[j]=rol32(w[j-3]^w[j-8]^w[j-14]^w[j-16],1);}
const[f,k]=fk(j,b,c,d);const temp=[rol32(a,5),f,e,k,w[j]].reduce(add32);[e,d,c,b,a]=[d,c,rol32(b,30),a,temp];}
...
var Endian=(function(Endian){Endian[Endian["Little"]=0]="Little";Endian[Endian["Big"]=1]="Big";return Endian;})({});function fk(index,b,c,d){if(index<20){return[(b&c)|(~b&d),0x5a827999];}
if(index<40){return[b^c^d,0x6ed9eba1];}
if(index<60){return[(b&c)|(b&d)|(c&d),0x8f1bbcdc];}
return[b^c^d,0xca62c1d6];}
function stringToWords32(str,endian){const size=(str.length+3)>>>2;const words32=[];for(let i=0;i<size;i++){words32[i]=wordAt(str,i*4,endian);}
return words32;}
function arrayBufferToWords32(buffer,endian){const size=(buffer.byteLength+3)>>>2;const words32=[];const view=new Uint8Array(buffer);for(let i=0;i<size;i++){words32[i]=wordAt(view,i*4,endian);}
return words32;}
...
else{if(yield this.needToRevalidate(req,cachedResponse)){this.idle.schedule(`revalidate(${cache.name}): ${req.url}`,()=>__awaiter(this,void 0,void 0,function*(){yield this.fetchAndCacheOnce(req);}));}
return cachedResponse;}}
const res=yield this.fetchAndCacheOnce(this.adapter.newRequest(req.url));return res.clone();}
else{return null;}});}
needToRevalidate(req,res){return __awaiter(this,void 0,void 0,function*(){if(res.headers.has('Cache-Control')){const cacheControl=res.headers.get('Cache-Control');const cacheDirectives=cacheControl.split(',').map(v=>v.trim()).map(v=>v.split('='));cacheDirectives.forEach(v=>v[0]=v[0].toLowerCase());const maxAgeDirective=cacheDirectives.find(v=>v[0]==='max-age');const cacheAge=maxAgeDirective?maxAgeDirective[1]:undefined;if(!cacheAge){return true;}
...
unhashedResources(){return __awaiter(this,void 0,void 0,function*(){const cache=yield this.cache;return(yield cache.keys()).map(request=>this.adapter.normalizeUrl(request.url)).filter(url=>!this.hashes.has(url));});}
...
yield this.fetchAndCacheOnce(req,false);}),Promise.resolve());if(updateFrom!==undefined){const metaTable=yield this.metadata;yield(yield updateFrom.previouslyCachedResources()).filter(url=>this.urls.indexOf(url)!==-1||this.patterns.some(pattern=>pattern.test(url))).reduce((previous,url)=>__awaiter(this,void 0,void 0,function*(){yield previous;const req=this.adapter.newRequest(url);const alreadyCached=((yield cache.match(req,this.config.cacheQueryOptions))!==undefined);if(alreadyCached){return;}
...
class DataGroup{constructor(scope,adapter,config,db,debugHandler,cacheNamePrefix){this.scope=scope;this.adapter=adapter;this.config=config;this.db=db;this.debugHandler=debugHandler;this._lru=null;this.patterns=config.patterns.map(pattern=>new RegExp(pattern));this.cache=adapter.caches.open(`${cacheNamePrefix}:${config.name}:cache`);this.lruTable=this.db.open(`${cacheNamePrefix}:${config.name}:lru`,config.cacheQueryOptions);this.ageTable=this.db.open(`${cacheNamePrefix}:${config.name}:age`,config.cacheQueryOptions);}
...
handleFetch(req,ctx){return __awaiter(this,void 0,void 0,function*(){if(!this.patterns.some(pattern=>pattern.test(req.url))){return null;}
...
cleanup(){return __awaiter(this,void 0,void 0,function*(){yield Promise.all([this.cache.then(cache=>this.adapter.caches.delete(cache.name)),this.ageTable.then(table=>this.db.delete(table.name)),this.lruTable.then(table=>this.db.delete(table.name)),]);});}
...
getCacheNames(){return __awaiter(this,void 0,void 0,function*(){const allGroupCacheNames=yield Promise.all([...this.assetGroups.map(group=>group.getCacheNames()),...this.dataGroups.map(group=>group.getCacheNames()),]);return[].concat(...allGroupCacheNames);});}
get appData(){return this.manifest.appData||null;}
acceptsTextHtml(req){const accept=req.headers.get('Accept');if(accept===null){return false;}
const values=accept.split(',');return values.some(value=>value.trim().toLowerCase()==='text/html');}}
const SW_VERSION='12.1.0';const DEBUG_LOG_BUFFER_SIZE=100;class DebugHandler{constructor(driver,adapter){this.driver=driver;this.adapter=adapter;this.debugLogA=[];this.debugLogB=[];}
handleFetch(req){return __awaiter(this,void 0,void 0,function*(){const[state,versions,idle]=yield Promise.all([this.driver.debugState(),this.driver.debugVersions(),this.driver.debugIdleState(),]);const msgState=`NGSW Debug Info:
...
class IdleScheduler{constructor(adapter,delay,maxDelay,debug){this.adapter=adapter;this.delay=delay;this.maxDelay=maxDelay;this.debug=debug;this.queue=[];this.scheduled=null;this.empty=Promise.resolve();this.emptyResolve=null;this.lastTrigger=null;this.lastRun=null;this.oldestScheduledAt=null;}
trigger(){var _a;return __awaiter(this,void 0,void 0,function*(){this.lastTrigger=this.adapter.time;if(this.queue.length===0){return;}
...
if(this.oldestScheduledAt===null){this.oldestScheduledAt=this.adapter.time;}}
...
onFetch(event){const req=event.request;const scopeUrl=this.scope.registration.scope;const requestUrlObj=this.adapter.parseUrl(req.url,scopeUrl);if(req.headers.has('ngsw-bypass')||/[?&]ngsw-bypass(?:[=&]|$)/i.test(requestUrlObj.search)){return;}
if(requestUrlObj.path===this.ngswStatePath){event.respondWith(this.debugger.handleFetch(req));return;}
if(this.state===DriverReadyState.SAFE_MODE){event.waitUntil(this.idle.trigger());return;}
if(requestUrlObj.origin.startsWith('http:')&&scopeUrl.startsWith('https:')){this.debugger.log(`Ignoring passive mixed content request: Driver.fetch(${req.url})`);return;}
if(req.cache==='only-if-cached'&&req.mode!=='same-origin'){if(!this.loggedInvalidOnlyIfCachedRequest){this.loggedInvalidOnlyIfCachedRequest=true;this.debugger.log(`Ignoring invalid request: 'only-if-cached' can be set only with 'same-origin' mode`,`Driver.fetch(${req.url}, cache: ${req.cache}, mode: ${req.mode})`);}
return;}
...
onClick(event){event.waitUntil(this.handleClick(event.notification,event.action));}
...
else{affectedClients.forEach(clientId=>this.clientVersionMap.set(clientId,this.latestHash));}
try{yield this.sync();}
...
const brokenHash=broken[0];const affectedClients=Array.from(this.clientVersionMap.entries()).filter(([clientId,hash])=>hash===brokenHash).map(([clientId])=>clientId);yield Promise.all(affectedClients.map((clientId)=>__awaiter(this,void 0,void 0,function*(){const client=yield this.scope.clients.get(clientId);client.postMessage({type:'UNRECOVERABLE_STATE',reason});})));});}
...
broadcast(msg){return __awaiter(this,void 0,void 0,function*(){const clients=yield this.scope.clients.matchAll();clients.forEach(client=>{client.postMessage(msg);});});}
debugState(){return __awaiter(this,void 0,void 0,function*(){return{state:DriverReadyState[this.state],why:this.stateMessage,latestHash:this.latestHash,lastUpdateCheck:this.lastUpdateCheck,};});}
debugVersions(){return __awaiter(this,void 0,void 0,function*(){return Array.from(this.versions.keys()).map(hash=>{const version=this.versions.get(hash);const clients=Array.from(this.clientVersionMap.entries()).filter(([clientId,version])=>version===hash).map(([clientId,version])=>clientId);return{hash,manifest:version.manifest,clients,status:'',};});});}
debugIdleState(){return __awaiter(this,void 0,void 0,function*(){return{queue:this.idle.taskDescriptions,lastTrigger:this.idle.lastTrigger,lastRun:this.idle.lastRun,};});}
safeFetch(req){return __awaiter(this,void 0,void 0,function*(){try{return yield this.scope.fetch(req);}
